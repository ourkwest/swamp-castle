  ...
  (let [aspect-ratio 2/3
    ...
    (->> (for [[theta-1 theta-2] sections]
           (let [inner-radius (* radius inner-radius-proportion)
             ...
             [(when (or last? second-last?) (intruct-poly (texture/shaded-stone-style 0) (+ (* layer 1000) 500) a b c d)) ; top
              (instruct-face a b b' a' light-angle-1) ; outer face
              (when last? (instruct-face b c c' b' light-angle-2))
              (instruct-face c d d' c' light-angle-3) ; inner face
              (when last? (instruct-face d a a' d' light-angle-4))]))
         (apply concat)
         #_(do-instructions g))))
