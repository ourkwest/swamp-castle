(ns board-game.support.draw
  (:require
    [board-game.support.util :as util]
    [board-game.support.protocols :as protocols]
...
(defn text [^Graphics2D g style ^String string x y]
  (when (protocols/prepare-for-draw style g)
    (let [font (.getFont g)
          ...
          ;bounds2 (.getBounds (doto (Area. (.getPixelBounds text-layout frc x y)) (.transform (.getTransform g))))
...
(defn text->shape
  ...
  ([^Graphics2D g style ^String text x y]
   ...
   (let [font (.getFont g)
         ...
         gx (- x (.getX bounds) (/ (.getWidth bounds) 2))
         gy (- y (.getY bounds) (/ (.getHeight bounds) 2))]
     (.getOutline text-layout (AffineTransform/getTranslateInstance (float (+ gx x-offset)) (float gy))))))
